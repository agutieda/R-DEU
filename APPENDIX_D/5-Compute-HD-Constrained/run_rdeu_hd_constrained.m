...
% - Assume R-DEU model with hyperbolic discounting where (ra,dr,pb) follow a 
%   multivariate distribution where the marginal of ra is normal, the marginal
%   of dr is a normal truncated at zero, and the marginal of pb is a beta 
%   distribution.
%
...
theta_0_pooled = [ ...
    % mu_ra, sig_ra, mu_dr, sig_dr, alf_pb , bet_pb rho_radr rho_rapb rho_drpb
   -0.041 , 0.264 , -19.29 , 3.88 , 11.07 , 1.38 , 0.01, -0.05 , 0
    ];
...
thetaBounds.max.alf_pb =  1000000;
thetaBounds.min.alf_pb =  0;
thetaBounds.max.bet_pb =  1000000;
thetaBounds.min.bet_pb =  0;
...
writetable(resTab,'./output/rdeu_pooled_mle_hd_constrained.csv','WriteRowNames',true);
