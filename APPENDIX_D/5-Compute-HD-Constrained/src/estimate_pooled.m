...
bNodes = ld_sequences(:,3)*(bMax-bMin) + bMin;
rNodes(rNodes==1) = 1 + 1e-12;

% Integration weights
intWeights =  ( (rMax-rMin)*(dMax-dMin)*(bMax-bMin)/nNodes ).* ones(nNodes,1);

% Store
estInput.rNodes = rNodes;
estInput.dNodes = dNodes;
estInput.bNodes = bNodes;
...
[theta_hat, ~, exitflag, ~, ~, hessian_loglike] = ...
    fminunc(obj_fun, estInput.theta_0, estInput.optFminunc_mle);
...
mom.mle.sd_dr      = MOM(9)  ;
mom.mle.Z_dr       = MOM(10) ;
mom.mle.mean_pb    = MOM(11) ;
mom.mle.median_pb  = MOM(12) ;
mom.mle.mode_pb    = MOM(13) ;
mom.mle.sd_pb      = MOM(14) ;
mom.mle.Z_pb       = MOM(15) ;
mom.mle.mu_ra_plus = MOM(16) ;
mom.mle.p_convex   = MOM(17) ;
mom.mle.iqr_ra     = MOM(18) ;
mom.mle.iqr_dr     = MOM(19) ;
mom.mle.iqr_pb     = MOM(20) ;
...
mom.se.mean_pb    = se_MOM_hat(11) ;
mom.se.median_pb  = se_MOM_hat(12) ;
mom.se.mode_pb    = se_MOM_hat(13) ;
mom.se.sd_pb      = se_MOM_hat(14) ;
mom.se.Z_pb       = se_MOM_hat(15) ;
mom.se.mu_ra_plus = se_MOM_hat(16) ;
mom.se.p_convex   = se_MOM_hat(17) ;
mom.se.iqr_ra     = se_MOM_hat(18) ;
mom.se.iqr_dr     = se_MOM_hat(19) ;
mom.se.iqr_pb     = se_MOM_hat(20) ;
...
fprintf(...
    'mu_ra:%g sig_ra:%g mu_da:%g sig_da:%g alf_pb:%g bet_pb:%g logLike:%g pdfCheck:%g \n', ...
