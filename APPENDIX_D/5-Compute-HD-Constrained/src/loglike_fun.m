...
badParameter = ...
    ...
    alf_pb >= estInput.thetaBounds.max.alf_pb || ...
    alf_pb <= estInput.thetaBounds.min.alf_pb || ...
    bet_pb >= estInput.thetaBounds.max.bet_pb || ...
    bet_pb <= estInput.thetaBounds.min.bet_pb || ...        
...
if badParameter == 1
    ...
    PDF_pb = pdf_pb( bNodes , alf_pb, bet_pb , bMin , bMax) ;
    CDF_ra = cdf_ra( rNodes , mu_ra , sig_ra , rMin , rMax) ;
    CDF_dr = cdf_dr( dNodes , mu_dr , sig_dr , dMin , dMax) ;
    CDF_pb = cdf_pb( bNodes , alf_pb, bet_pb , bMin , bMax) ;
